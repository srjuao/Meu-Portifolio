function initLiquidEther(container, userOptions = {}) {
    ...
    const props = { ...defaultOptions, ...userOptions };
    ...
    class ExternalForce extends ShaderPass {
        ...
        update(opts) {
            ...
            const centerX = Math.min(
                Math.max(Mouse.coords.x, -1 + cursorSizeX + opts.cellScale.x * 2),
                1 - cursorSizeX - opts.cellScale.x * 2
            );
            const centerY = Math.min(
                Math.max(Mouse.coords.y, -1 + cursorSizeY + opts.cellScale.y * 2),
                1 - cursorSizeY - opts.cellScale.y * 2
            );
            const uniforms = this.mouse.material.uniforms;
            uniforms.force.value.set(forceX, forceY);
            uniforms.center.value.set(centerX, centerY);
            uniforms.scale.value.set(opts.cursor_size, opts.cursor_size);
            super.update();
        }
    }
    ...
    // Removed lines that were overriding CSS position
    container.style.overflow = 'hidden';

    const webgl = new WebGLManager(container, props);
    webgl.start();
